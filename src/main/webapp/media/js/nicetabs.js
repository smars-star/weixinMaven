...
function initMenu() {
    ...
    for (i = 0; i < uls.length; i++) {
        if (uls[i].className == "menuList") {
            decorateTabMenu(uls[i]);
        }
    }
}

function decorateTabMenu(menu) {
...
function activateMenu(items) {
    ...
    if (found > 1) {
        ...
        for (var i=0; i < items.length; i++) {
            var url = items[i].firstChild.getAttribute("href");
            var current = document.location.toString();
			//if (current.indexOf(url) != -1){  //Modified By wangf
            if (!(current.indexOf(url) == -1 && currentTabURL.indexOf(url) == -1)) {  //Modified By wangf
			  ...
			  items[i].className ="";//add By wangf
			} //add By wangf
        }
    }
}
...
//==========================================================================
//增加全局参数用于判断标签页面的url为javascript时的页面选择问题（Added By Wangf）
//==========================================================================
var currentTabURL=document.location.toString();
// =========================================================================
//                          Cookie functions
// =========================================================================
